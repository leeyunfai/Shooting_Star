...
// first edited into SWTFT: downloaded from http://www.xcluma.com/how-to-use-2.4inch-tft-lcd-mcufriend
// second edit for LCD_id:5408 by jiekai from SWTFT for MA2012
...
TFT5408::TFT5408() :  Adafruit_GFX(TFTWIDTH, TFTHEIGHT) {
...
void TFT5408::begin(uint16_t id) {
  uint8_t i = 0;
...
void TFT5408::setAddrWindow(int x1, int y1, int x2, int y2) {
  ...
  CS_ACTIVE;
    ...
    int x, y, t;
    switch(rotation) {
    case 2: //change from default to case 3 //jiekai
     t  = x1;  //added by jiekai
     x1 = TFTWIDTH  - 1 - x2; //added by jiekai
     x2 = TFTWIDTH  - 1 - t; //added by jiekai
      x  = x1;
      y  = y1;
      break;
     case 1:
      ...
      y2 = x2;
      x2 = TFTWIDTH  - 1 - t;
      t  = y1; //added by jiekai
      y1 = TFTHEIGHT - 1 - y2; //added by jiekai
      y2 = TFTHEIGHT - 1 - t; //added by jiekai
      x  = x2;
      y  = y1;
      break;
     default: //change from case 2 to default //jiekai
      // t  = x1;  //comment by jiekai
      // x1 = TFTWIDTH  - 1 - x2; //comment by jiekai
      // x2 = TFTWIDTH  - 1 - t; //comment by jiekai
      t  = y1;
      y1 = TFTHEIGHT - 1 - y2;
      y2 = TFTHEIGHT - 1 - t;
      x  = x2;
...
void TFT5408::flood(uint16_t color, uint32_t len) {
  uint16_t blocks;
  color = color ^ 0xFFFF; //added by jiekai
...
void TFT5408::drawFastHLine(int16_t x, int16_t y, int16_t length,
  uint16_t color)
{
  int16_t x2;
...
void TFT5408::drawFastVLine(int16_t x, int16_t y, int16_t length,
  uint16_t color)
{
  int16_t y2;
...
void TFT5408::fillRect(int16_t x1, int16_t y1, int16_t w, int16_t h, 
  uint16_t fillcolor) {
  int16_t  x2, y2;
...
void TFT5408::fillScreen(uint16_t color) {
...
void TFT5408::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
void TFT5408::pushColors(uint16_t *data, uint8_t len, boolean first) {
  uint16_t color;
  uint8_t  hi, lo;
  CS_ACTIVE;
...
void TFT5408::setRotation(uint8_t x) {
...
uint16_t TFT5408::readPixel(int16_t x, int16_t y) {
...
uint16_t TFT5408::color565(uint8_t r, uint8_t g, uint8_t b) {
  return ((r & 0xF8) << 8) | ((g & 0xFC) << 3) | (b >> 3);
}

//////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////
void TFT5408::writeA16B3(uint16_t address, uint8_t ub, uint8_t mb, uint8_t lb){
	uint8_t hi , lo;
	CS_ACTIVE;
...
void TFT5408::drawLine3Byte(int16_t x0, int16_t y0,
...
void TFT5408::drawPixel3(int16_t x, int16_t y, uint8_t r, uint8_t g, uint8_t b)
{
if((x < 0) || (y < 0) || (x >= _width) || (y >= _height)) return;
...
#ifndef write8
void TFT5408::write8(uint8_t value) {
  write8inline(value);
}
#endif

#ifdef read8isFunctionalized
uint8_t TFT5408::read8fn(void) {
  uint8_t result;
  read8inline(result);
  return result;
}
...
#ifndef setWriteDir
void TFT5408::setWriteDir(void) {
  setWriteDirInline();
}
#endif

#ifndef setReadDir
void TFT5408::setReadDir(void) {
  setReadDirInline();
}
#endif

#ifndef writeRegister8
void TFT5408::writeRegister8(uint8_t a, uint8_t d) {
  writeRegister8inline(a, d);
}
#endif

#ifndef writeRegister16
void TFT5408::writeRegister16(uint16_t a, uint16_t d) {
  writeRegister16inline(a, d);
}
#endif

#ifndef writeRegisterPair
void TFT5408::writeRegisterPair(uint8_t aH, uint8_t aL, uint16_t d) {
